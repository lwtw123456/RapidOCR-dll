...
| `only_text` | `bool` | 是否直接返回纯文本（按行拼接），而不是 JSON 结果 | `false` |

`merge_code_lines` 适合代码编辑器截图、终端截图等文本场景。  
当 `only_text = true` 时，接口返回值不再是 JSON，而是 UTF-8 纯文本字符串。

------------------------------------------------------------------------

## 🧾 返回数据结构

默认情况下，接口成功或失败都会返回 UTF-8 编码的 JSON 字符串。
...
时，接口成功时将直接返回 UTF-8 纯文本字符串（按行拼接），不再返回 JSON 结构。
